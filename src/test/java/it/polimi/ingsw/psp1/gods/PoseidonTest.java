...
public class PoseidonTest {
    ...
    public void onYourBuild_normalBehaviourBuild_shouldBuild3TimesWithUnmovedWorker() {
        ...
        player.getPower().onYourMove(w1, new Point(1, 2), game);

        assertFalse(player.getTurnState().shouldShowInteraction());

        player.getPower().onYourBuild(w1, new Point(1, 1), game);
        ...
        player.getPower().onYourBuild(w2, new Point(3, 4), game);
        player.getPower().onYourBuild(w2, new Point(4, 3), game);
        player.getPower().onYourBuild(w2, new Point(4, 4), game);
    ...
    public void onYourBuild_normalBehaviourBuild_shouldEndTurnAfterBuild() {
        ...
        player.getPower().onYourMove(w2, new Point(1, 2), game);
        player.getPower().onYourBuild(w2, new Point(1, 1), game);
